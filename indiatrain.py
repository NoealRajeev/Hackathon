    ...
    for filename in os.listdir(xml_dir):
        if filename.endswith(".xml"):
            ...
            for obj in soup.find_all("object"):
                ...
                if name.lower() == "pothole":  # Filter annotations for potholes
                    bbox = obj.find("bndbox")
                    bbox_coords = [int(bbox.find(coord).text) for coord in ["xmin", "ymin", "xmax", "ymax"]]
                    annotation = {
                        "id": annotation_id,
                        "image_id": image_id,
                        "category_id": 1,  # Assuming category ID for potholes is 1
                        "bbox": bbox_coords,
...
images_dir = r"C:\Users\rupal\Desktop\India\train\images"
annotations_dir = r"C:\Users\rupal\Desktop\India\train\annotations\xmls"
...
# Perform object detection on train images and draw bounding boxes
for filename in os.listdir(images_dir):
    if filename.endswith(".jpg"):
        image_path = os.path.join(images_dir, filename)
        detected_objects = detect_objects(image_path)
        print("Detected potholes:", detected_objects)
